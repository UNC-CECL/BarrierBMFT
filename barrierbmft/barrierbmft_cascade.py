...
De-couples Barrier3D (Reeves et al., 2021) and the BMFT-C model (python version) for use in CASCADE
Changes to this file (compared to original barrierbmft file) include:
...
3. split the update function into two different functions
    function 1 (update) includes everything before advancing b3d
        ...
        returns the pre-b3d domain because that gets used later (now in function 2)
        time_step is set to b3d_instance.time_index - 1
    function 2 (update_marsh) includes everything after advancing b3d
        requires inputs: self, pre-b3d domain, and b3d instance
        returns nothing, just updates the class variables
        time_step is now set to b3d_instance.time_index - 2
    b3d instance is now an input into both marsh functions
4. removing the call to advance Barrier3D by a single time step
...
class BarrierBMFT:
    ...
    def __init__(
        ...
        self._x_s_offset = initial_BB_subaerial_width - (b3d_instance.InteriorWidth_AvgTS[-1] * 10)  # Initial location of B in PyBMFT-C relative to x_s_initial in Barrier3D
    ...
    def update(self, b3d_instance):
        ...
        """
        bmftc classes are the same time_index as b3d time_index after they update, but in the original code, the 
        time_step variable used here was not tied to a class time index. therefore, it did not update when classes 
        were updated
        Example:
        when time_step = 0:
        bmftc classes are at time_index = 1 (now after they update)
        b3d class is at time_index = 1
        therefore, time_step should be self.bmftc.time_index - 1 or b3d_instance.time_index - 1
        """
        time_step = b3d_instance.time_index - 1
        ...
        b3d_instance._BayDepth = np.mean([self._bmftc_ML.db, self._bmftc_BB.db]) / 10

        # Adjust bay routing width in Barrier3D if fetch in PyBMFTC becomes narrower than 100 m
        if self._bmftc_ML.bfo < 100:
            b3d_instance._bay_routing_width = int(math.floor(self._bmftc_ML.bfo / 10))
        else:
            b3d_instance._bay_routing_width = 15
        ...
        if addRows > 0:
            ...
            Marsh_Addition = np.ones([addRows, b3d_instance.BarrierLength]) * -b3d_instance._BayDepth
            Zero_Addition = np.zeros([addRows, b3d_instance.BarrierLength])
            NewDomain = np.vstack([b3d_instance.InteriorDomain, Marsh_Addition])
            # Update size of shrub domains, too
            b3d_instance._ShrubDomainFemale = np.vstack([b3d_instance.ShrubDomainFemale, Zero_Addition])
            b3d_instance._ShrubDomainMale = np.vstack([b3d_instance.ShrubDomainMale, Zero_Addition])
            b3d_instance._ShrubDomainDead = np.vstack([b3d_instance.ShrubDomainDead, Zero_Addition])
            b3d_instance._ShrubPercentCover = np.vstack([b3d_instance.ShrubPercentCover, Zero_Addition])
            b3d_instance._DeadPercentCover = np.vstack([b3d_instance.DeadPercentCover, Zero_Addition])
            b3d_instance._BurialDomain = np.vstack([b3d_instance.BurialDomain, Zero_Addition])
            b3d_instance._ShrubDomainAll = b3d_instance.ShrubDomainFemale + b3d_instance.ShrubDomainMale
        elif addRows < 0:
            # Update interior domain size
            NewDomain = b3d_instance.InteriorDomain[:addRows, :]
            # Update size of shrub domains, too
            b3d_instance._ShrubDomainFemale = b3d_instance.ShrubDomainFemale[:addRows, :]
            b3d_instance._ShrubDomainMale = b3d_instance.ShrubDomainMale[:addRows, :]
            b3d_instance._ShrubDomainDead = b3d_instance.ShrubDomainDead[:addRows, :]
            b3d_instance._ShrubPercentCover = b3d_instance.ShrubPercentCover[:addRows, :]
            b3d_instance._DeadPercentCover = b3d_instance.DeadPercentCover[:addRows, :]
            b3d_instance._BurialDomain = b3d_instance.BurialDomain[:addRows, :]
            b3d_instance._ShrubDomainAll = b3d_instance._ShrubDomainFemale + b3d_instance._ShrubDomainMale
        else:
    ...
    def update_marsh(self, new_domain, b3d_instance):
        ...
        end_b3d = end_b3d + (b3d_instance.RSLR[time_step] * 10)  # Offset sea-level rise from Barrier3D so that it isn't counted twice (i.e. RSLR already taken into account in PyBMFT-C)
